...
import {
  ...
  SERUM_PROGRAM_ID_V3,
} from '../lib/raydium/util/ids.js'
import { LiquidityPoolInfo } from '../lib/raydium/util/pools.js'
import {
  createAmmAuthority,
  findAssociatedTokenAddress,
} from '../lib/raydium/util/web3.js'
import {
  calcSwapAmountForDesiredPrice,
  checkOptions,
...
import { NATIVE_SOL, TokenInfo, TOKENS } from '../lib/raydium/util/tokens.js'
import { MarketAccountsBalance } from '../lib/raydium/types/marketAccountsBalance.js'
...
async function printMarketAccountsBalance(
  payerAccountBalance: MarketAccountsBalance
) {
  console.log(
    'BaseMint ATA Address:',
    payerAccountBalance.tokenA.publicKey.toString()
  )
  console.log(
    'Balance:',
    payerAccountBalance.tokenA.balance
      ? payerAccountBalance.tokenA.balance.uiAmount
      : 'Account Not Created!'
  )

  console.log(
    'QuoteMint ATA Address:',
    payerAccountBalance.tokenB.publicKey.toString()
  )
  console.log(
    'Balance:',
    payerAccountBalance.tokenB.balance
      ? payerAccountBalance.tokenB.balance.uiAmount
      : 'Account Not Created!'
  )
}

async function getMarketAccountsBalance(
  owner: PublicKey,
  mintA: PublicKey,
  mintB: PublicKey,
  connection: Connection
): Promise<MarketAccountsBalance> {
  const ataTokenA = await findAssociatedTokenAddress(owner, mintA)
  const ataTokenB = await findAssociatedTokenAddress(owner, mintB)

  const ataAccountInfoTokenA = await connection.getAccountInfo(
    ataTokenA,
    'finalized'
  )

  const ataAccountInfoTokenB = await connection.getAccountInfo(
    ataTokenB,
    'finalized'
  )
...
async function showMarket(marketPubKey: string, option: Option) {
  checkOptions(option, 'network')
  const networkName = getNetworkName(option)
  const connection = getConnection(networkName)
  ...
  const market = await getSerumMarket(
    marketAddress,
    connection,
    SERUM_PROGRAM_ID_V3[networkName]
  )
...
async function calculateMarketPrice(marketPubKey: string, option: Option) {
  checkOptions(option, 'network')
  const networkName = getNetworkName(option)
  const connection = getConnection(networkName)
  ...
  const fillPrices: FillPrice[] = filledBids.map((a) => {
    return {
      price: a.price,
      size: a.size,
    }
...
async function placeOrder(
  ...
  const marketAddress = new PublicKey(marketPubKey)
  const market = await MarketSerum.load(
    connection,
    marketAddress,
    {},
    SERUM_PROGRAM_ID_V3[networkName]
  ...
  const ataTokenA = await findAssociatedTokenAddress(
    payer.publicKey,
    market.baseMintAddress
  )

  const ataTokenB = await findAssociatedTokenAddress(
    payer.publicKey,
  ...
  for (const openOrders of await market.findOpenOrdersAccountsForOwner(
    ...
    if (
      ...
      const settle = await market.settleFunds(
        connection,
        owner,
        openOrders,
        baseTokenAccount,
...
async function showMarketATAsBalance(
  marketPubKey: string,
  option: Option
): Promise<void> {
  ...
  const payerAccountBalance: MarketAccountsBalance =
    await getMarketAccountsBalance(
      payer.publicKey,
      market.baseMint,
      market.quoteMint,
      connection
...
async function showPoolATAsBalance(
  ammId: string,
  option: Option
): Promise<void> {
  checkOptions(option, 'network', 'payer')
  ...
  const ataTokenLP = await findAssociatedTokenAddress(
    payer.publicKey,
    ammState.lpMint
  )
  ...
  const ataAccountInfoTokenLP = await connection.getAccountInfo(
    ataTokenLP,
    'finalized'
  )
  ...
  console.log(
    'Balance:',
    ataBalanceTokenA ? ataBalanceTokenA.uiAmountString : 'Account Not Created!'
  )
  console.log('Quote Token ATA:', ataTokenB.toString())
  console.log(
    'Balance:',
    ataBalanceTokenB ? ataBalanceTokenB.uiAmountString : 'Account Not Created!'
  )
...
async function createRayPool(
  marketPubKey: string,
  tokenAStartingBalance: string,
  tokenBStartingBalance: string,
  option: Option
): Promise<void> {
  checkOptions(option, 'network', 'payer')
  const networkName = getNetworkName(option)
  const connection = getConnection(networkName)
  const payer = await createKeypairFromFile(option['payer'])
  const serumProgramId: PublicKey = SERUM_PROGRAM_ID_V3[networkName]
  const raydiumProgramId: PublicKey = RAYDIUM_PROGRAM_ID_V4[networkName]
  ...
  if (
    ...
    console.log(
      'Unable to create AMM; not enough funds for initial LP amount specified.'
    )
    printMarketAccountsBalance(payerAccountBalance)
  }
}
...
export async function getLiquidityPoolInfo(
  ...
  const tokenABalance = new TokenAmount(
  ...
  const tokenBBalance = new TokenAmount(
...
async function lpDeposit(
  ...
  if (ataAccountInfoTokenA && ataAccountInfoTokenB && ataAccountInfoTokenLP) {
    const ataBalanceTokenA = (
      await connection.getTokenAccountBalance(ataTokenA)
    ).value
    const ataBalanceTokenB = (
      await connection.getTokenAccountBalance(ataTokenB)
    ).value
    if (
      ataBalanceTokenA.uiAmount > parseInt(amountTokenA) &&
...
async function getLPBalance(ammId: string, option: Option): Promise<void> {
  checkOptions(option, 'network', 'payer')
  const networkName = getNetworkName(option)
  const connection = getConnection(networkName)
  const payer = await createKeypairFromFile(option['payer'])
...
async function getPrice(ammIdStr: string, option: Option) {
  checkOptions(option, 'network')
  const networkName = getNetworkName(option)
  const connection = getConnection(networkName)
  ...
  // base token amount
  const baseReserve = new Decimal(poolInfo.baseReserve.toString()).div(
    new Decimal(10).pow(new Decimal(poolInfo.baseDecimals))
  )

  // quote token amount
  const quoteReserve = new Decimal(poolInfo.quoteReserve.toString()).div(
    new Decimal(10).pow(new Decimal(poolInfo.quoteDecimals))
  )
...
async function setPrice(
  ...
  // base token amount
  const baseReserve = new Decimal(newPoolInfo.baseReserve.toNumber()).div(
    new Decimal(10).pow(new Decimal(coin.decimals))
  )

  // quote token amount
  const quoteReserve = new Decimal(newPoolInfo.quoteReserve.toString()).div(
    new Decimal(10).pow(new Decimal(pc.decimals))
  )
  ...
  const tradeFee =
    (poolInfo.fees &&
      new Fee(
        poolInfo.fees.swapFeeNumerator,
        poolInfo.fees.swapFeeDenominator
      )) ||
    new Fee(0, 1)
  const swapAmount = calcSwapAmountForDesiredPrice(
    'raydium',
    baseReserve,
    poolInfo.coin.decimals,
    quoteReserve,
    poolInfo.coin.decimals,
    desiredPrice,
    tradeFee
  ...
  if (swapAmount) {
    ...
    if (!('quoteOnly' in option)) {
...

      {
        ...
        // base token amount
        const baseReserve = new Decimal(baseBalanceStr).div(
          new Decimal(10).pow(new Decimal(coin.decimals))
        )

        // quote token amount
        const quoteReserve = new Decimal(quoteBalanceStr).div(
          new Decimal(10).pow(new Decimal(pc.decimals))
        )
    ...
    console.log(
      'Unable to determine swap amount required to establish price:',
      desiredPrice.toString()
    )
  }
}

export function addRaydiumCommands(program: Command): void {
  ...
  raydium
    .command(
      'createPool <marketPubKey> <tokenAStartingBalance> <tokenBStartingBalance>'
    )
