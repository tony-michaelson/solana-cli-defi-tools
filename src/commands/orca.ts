...
async function createPool(
  mintA: string,
  payerTokenAccountAStr: string,
  mintB: string,
  payerTokenAccountBStr: string,
  option: Option
) {
  ...
  const swap = await TokenSwap.createTokenSwap(
    ...
    0 // 0 for ConstantProduct & 2 for Stable
  )
...
async function showPool(tokenSwapID: string, option: Option) {
  ...
  const tokenAccountBalA = (
    await connection.getTokenAccountBalance(tokenSwap.tokenAccountA)
  ).value.uiAmount
  const tokenAccountBalB = (
    await connection.getTokenAccountBalance(tokenSwap.tokenAccountB)
  ).value.uiAmount
...
async function deposit(tokenSwapID: string, tokenQty: string, option: Option) {
  checkOptions(option, 'network', 'payer')
  const networkName = getNetworkName(option)
  const connection = getConnection(networkName)
  const payer = await createKeypairFromFile(option['payer'])
  ...
  const inputToken =
    option['token'] === 'A' ? orcaPool.getTokenA() : orcaPool.getTokenB()
  ...
  if (!('quoteOnly' in option)) {
    const poolDepositPayload = await orcaPool.deposit(
      payer,
      maxTokenAIn,
      maxTokenBIn,
...
async function withdraw(tokenSwapID: string, tokenQty: string, option: Option) {
  ...
  const withdrawTokenAmount =
    'all' in option ? await orcaPool.getLPBalance(payer.publicKey) : tokenAmount
  const withdrawTokenMint = orcaPool.getPoolTokenMint()

  const { maxPoolTokenAmountIn, minTokenAOut, minTokenBOut } =
    await orcaPool.getWithdrawQuote(withdrawTokenAmount, withdrawTokenMint)
  ...
  if (!('quoteOnly' in option)) {
    ...
    const poolWithdrawPayload = await orcaPool.withdraw(
      payer,
...
async function swap(tokenSwapID: string, tokenQty: string, option: Option) {
  ...
  const fromCoin: orca.OrcaPoolToken =
    option['direction'] === 'AtB' ? orcaPool.getTokenA() : orcaPool.getTokenB()
  const toCoin: orca.OrcaPoolToken =
    option['direction'] === 'AtB' ? orcaPool.getTokenB() : orcaPool.getTokenA()
...
async function getPrice(tokenSwapID: string, option: Option) {
  checkOptions(option, 'network', 'payer')
  const networkName = getNetworkName(option)
  const connection = getConnection(networkName)
  const payer = await createKeypairFromFile(option['payer'])
...
async function setPrice(tokenSwapID: string, price: string, option: Option) {
  checkOptions(option, 'network', 'payer')
  const networkName = getNetworkName(option)
  const connection = getConnection(networkName)
  const payer = await createKeypairFromFile(option['payer'])
  ...
  const swapAmount = calcSwapAmountForDesiredPrice(
    'orca',
    baseBalance,
    baseBalanceLookup.value.decimals,
    quoteBalance,
    quoteBalanceLookup.value.decimals,
    desiredPrice,
    tradeFee,
    ownerFee
  )
...
export function addOrcaCommands(program: Command): void {
  const ocra = program.command('orca')

  ocra
    .command(
      'createPool <mintA> <payerTokenAccountA> <mintB> <payerTokenAccountB>'
    )
