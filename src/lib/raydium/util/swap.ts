...
export function getSwapOutAmount(
  ...
  if (fromCoinMint === coin.mintAddress && toCoinMint === pc.mintAddress) {
    ...
    return {
      ...
      newBaseBal: parseFloat(
        new TokenAmount(denominator, coin.decimals).fixed()
      ),
    ...
    return {
      ...
      newBaseBal: parseFloat(
        new TokenAmount(outBalance, coin.decimals).fixed()
      ),
      newQuoteBal: parseFloat(
        new TokenAmount(denominator, pc.decimals).fixed()
      ),
      priceImpact,
    }
  }
...
export async function swap(
  ...
  const from =
    poolInfo.coin.mintAddress === fromCoinMint ? poolInfo.coin : poolInfo.pc
  const to =
    poolInfo.pc.mintAddress === toCoinMint ? poolInfo.pc : poolInfo.coin
