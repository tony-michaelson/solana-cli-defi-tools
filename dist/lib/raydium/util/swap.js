import { _OPEN_ORDERS_LAYOUT_V2, OpenOrders, } from '@project-serum/serum/lib/market.js';
import { closeAccount } from '@project-serum/serum/lib/token-instructions.js';
import { LAMPORTS_PER_SOL, PublicKey, Transaction, TransactionInstruction, } from '@solana/web3.js';
// @ts-ignore
import { nu64, struct, u8 } from 'buffer-layout';
// eslint-disable-next-line
import { TokenAmount } from './safe-math.js';
import { createAssociatedTokenAccountIfNotExist, createAtaSolIfNotExistAndWrap, createProgramAccountIfNotExist, createTokenAccountIfNotExist, findProgramAddress, mergeTransactions, sendTransaction, } from './web3.js';
import { MEMO_PROGRAM_ID, ROUTE_SWAP_PROGRAM_ID, SERUM_PROGRAM_ID_V3, SYSTEM_PROGRAM_ID, TOKEN_PROGRAM_ID, } from './ids.js';
import { getBigNumber } from './layouts.js';
// eslint-disable-next-line
import { getTokenByMintAddress, NATIVE_SOL, TOKENS } from './tokens.js';
export function getOutAmount(market, asks, bids, fromCoinMint, toCoinMint, amount, slippage) {
    ...
    if (fromMint === market.quoteMintAddress.toBase58() &&
...
export function getSwapOutAmount(poolInfo, fromCoinMint, toCoinMint, amount, slippage) {
    ...
    if (fromCoinMint === coin.mintAddress && toCoinMint === pc.mintAddress) {
        ...
        return {
            ...
            newBaseBal: parseFloat(new TokenAmount(denominator, coin.decimals).fixed()),
            newQuoteBal: parseFloat(new TokenAmount(outBalance, pc.decimals).fixed()),
            priceImpact,
        };
    }
    else {
        ...
        return {
            ...
            amountOutWithSlippage: new TokenAmount(amountOutWithSlippage, pc.decimals),
            newBaseBal: parseFloat(new TokenAmount(outBalance, coin.decimals).fixed()),
            newQuoteBal: parseFloat(new TokenAmount(denominator, pc.decimals).fixed()),
            priceImpact,
        };
    }
}
export function getSwapInAmount(poolInfo, fromCoinMint, toCoinMint, amount, slippage) {
    const { coin, pc, fees } = poolInfo;
    const { swapFeeNumerator, swapFeeDenominator } = fees;
...
export function getSwapOutAmountStable(poolInfo, fromCoinMint, toCoinMint, amount, slippage) {
...
export function getSwapRouter(poolInfos, fromCoinMint, toCoinMint) {
    ...
    for (const p of poolInfos) {
        if (!(p.version === 4 && p.status === 1))
            continue;
        if ([fromCoinMint, toCoinMint].includes(p.coin.mintAddress) &&
            routerCoinDefault.includes(p.pc.symbol)) {
            avaPools.push(p);
        }
        else if ([fromCoinMint, toCoinMint].includes(p.pc.mintAddress) &&
...
export async function swap(connection, wallet, poolInfo, fromCoinMint, toCoinMint, fromTokenAccount, toTokenAccount, aIn, aOut, wsolAddress) {
    ...
    // }
    const from = poolInfo.coin.mintAddress === fromCoinMint ? poolInfo.coin : poolInfo.pc;
    const to = poolInfo.pc.mintAddress === toCoinMint ? poolInfo.pc : poolInfo.coin;
    ...
    if (fromCoinMint === NATIVE_SOL.mintAddress) {
        wrappedSolAccount = await createTokenAccountIfNotExist(connection, wrappedSolAccount.toString(), owner, TOKENS.WSOL.mintAddress, getBigNumber(amountIn.wei) + 1e7, transaction, signers);
    }
    else {
...
export async function preSwapRoute(connection, wallet, fromMint, fromTokenAccount, middleMint, middleTokenAccount, toMint, toTokenAccount, needWrapAmount) {
    const transaction = new Transaction();
    const signers = [];
    const owner = wallet.publicKey;
...
export async function swapRoute(connection, wallet, poolInfoA, poolInfoB, routerInfo, fromTokenAccount, middleTokenAccount, toTokenAccount, aIn, aOut, raydiumProgramId) {
    const transaction = new Transaction();
    const owner = wallet.publicKey;
    ...
    transaction.add(routeSwapInInstruction(new PublicKey(ROUTE_SWAP_PROGRAM_ID), raydiumProgramId, new PublicKey(poolInfoA.ammId), new PublicKey(poolInfoB.ammId), new PublicKey(poolInfoA.ammAuthority), new PublicKey(poolInfoA.ammOpenOrders), new PublicKey(poolInfoA.ammTargetOrders), new PublicKey(poolInfoA.poolCoinTokenAccount), new PublicKey(poolInfoA.poolPcTokenAccount), new PublicKey(poolInfoA.serumProgramId), new PublicKey(poolInfoA.serumMarket), new PublicKey(poolInfoA.serumBids), new PublicKey(poolInfoA.serumAsks), new PublicKey(poolInfoA.serumEventQueue), new PublicKey(poolInfoA.serumCoinVaultAccount), new PublicKey(poolInfoA.serumPcVaultAccount), new PublicKey(poolInfoA.serumVaultSigner), newFromTokenAccount, newMiddleTokenAccount, publicKey, owner, Math.floor(getBigNumber(amountIn.toWei()))), routeSwapOutInstruction(new PublicKey(ROUTE_SWAP_PROGRAM_ID), raydiumProgramId, new PublicKey(poolInfoA.ammId), new PublicKey(poolInfoB.ammId), new PublicKey(poolInfoB.ammAuthority), new PublicKey(poolInfoB.ammOpenOrders), new PublicKey(poolInfoB.ammTargetOrders), new PublicKey(poolInfoB.poolCoinTokenAccount), new PublicKey(poolInfoB.poolPcTokenAccount), new PublicKey(poolInfoB.serumProgramId), new PublicKey(poolInfoB.serumMarket), new PublicKey(poolInfoB.serumBids), new PublicKey(poolInfoB.serumAsks), new PublicKey(poolInfoB.serumEventQueue), new PublicKey(poolInfoB.serumCoinVaultAccount), new PublicKey(poolInfoB.serumPcVaultAccount), new PublicKey(poolInfoB.serumVaultSigner), newMiddleTokenAccount, newToTokenAccount, publicKey, owner, Math.floor(getBigNumber(amountOut.toWei()))));
    return await sendTransaction(connection, wallet, transaction);
}
export async function swapRouteOld(connection, wallet, poolInfoA, poolInfoB, routerInfo, fromTokenAccount, middleTokenAccount, toTokenAccount, aIn, aMiddle, aOut) {
    const transaction = new Transaction();
    const owner = wallet.publicKey;
    const fromCoinMint = routerInfo.route[0].mintA;
    const toCoinMint = routerInfo.route[1].mintB;
    ...
    if (fromCoinMint === NATIVE_SOL.mintAddress) {
        wrappedSolAccount = await createTokenAccountIfNotExist(connection, wrappedSolAccount.toString(), owner, TOKENS.WSOL.mintAddress, getBigNumber(amountIn.wei) + 1e7, transaction, []);
    }
    if (middleMint === NATIVE_SOL.mintAddress) {
        wrappedSolAccount2 = await createTokenAccountIfNotExist(connection, wrappedSolAccount2.toString(), owner, TOKENS.WSOL.mintAddress, 1e7, transaction, []);
    }
    if (toCoinMint === NATIVE_SOL.mintAddress) {
        wrappedSolAccount3 = await createTokenAccountIfNotExist(connection, wrappedSolAccount3.toString(), owner, TOKENS.WSOL.mintAddress, 1e7, transaction, []);
    }
    const newFromTokenAccount = await createAssociatedTokenAccountIfNotExist(fromTokenAccount, owner, fromMint, transaction);
    const newMiddleTokenAccount = await createAssociatedTokenAccountIfNotExist(middleTokenAccount, owner, middleMint, transaction);
    const newToTokenAccount = await createAssociatedTokenAccountIfNotExist(toTokenAccount, owner, toMint, transaction);
    transaction.add(swapInstruction(new PublicKey(poolInfoA.programId), new PublicKey(poolInfoA.ammId), new PublicKey(poolInfoA.ammAuthority), new PublicKey(poolInfoA.ammOpenOrders), new PublicKey(poolInfoA.ammTargetOrders), new PublicKey(poolInfoA.poolCoinTokenAccount), new PublicKey(poolInfoA.poolPcTokenAccount), new PublicKey(poolInfoA.serumProgramId), new PublicKey(poolInfoA.serumMarket), new PublicKey(poolInfoA.serumBids), new PublicKey(poolInfoA.serumAsks), new PublicKey(poolInfoA.serumEventQueue), new PublicKey(poolInfoA.serumCoinVaultAccount), new PublicKey(poolInfoA.serumPcVaultAccount), new PublicKey(poolInfoA.serumVaultSigner), wrappedSolAccount ?? newFromTokenAccount, wrappedSolAccount2 ?? newMiddleTokenAccount, owner, Math.floor(getBigNumber(amountIn.toWei())), Math.floor(getBigNumber(amountMiddle.toWei()))), swapInstruction(new PublicKey(poolInfoB.programId), new PublicKey(poolInfoB.ammId), new PublicKey(poolInfoB.ammAuthority), new PublicKey(poolInfoB.ammOpenOrders), new PublicKey(poolInfoB.ammTargetOrders), new PublicKey(poolInfoB.poolCoinTokenAccount), new PublicKey(poolInfoB.poolPcTokenAccount), new PublicKey(poolInfoB.serumProgramId), new PublicKey(poolInfoB.serumMarket), new PublicKey(poolInfoB.serumBids), new PublicKey(poolInfoB.serumAsks), new PublicKey(poolInfoB.serumEventQueue), new PublicKey(poolInfoB.serumCoinVaultAccount), new PublicKey(poolInfoB.serumPcVaultAccount), new PublicKey(poolInfoB.serumVaultSigner), wrappedSolAccount2 ?? newMiddleTokenAccount, wrappedSolAccount3 ?? newToTokenAccount, owner, Math.floor(getBigNumber(amountMiddle.toWei())), Math.floor(getBigNumber(amountOut.toWei()))));
    if (wrappedSolAccount) {
        transaction.add(closeAccount({
            source: wrappedSolAccount,
            destination: owner,
...
export async function place(connection, wallet, market, fromCoinMint, toCoinMint, fromTokenAccount, toTokenAccount, side, maxInAllow, amountOut, worstPrice) {
    if (!side || !worstPrice)
        throw new Error('Miss side');
    const transaction = new Transaction();
    const signers = [];
    const owner = wallet.publicKey;
    ...
    openOrdersAccounts.length === 0
        ? null
        : openOrdersAccounts[0].address.toBase58(), owner, new PublicKey(SERUM_PROGRAM_ID_V3), null, _OPEN_ORDERS_LAYOUT_V2, transaction, signers);
    let wrappedSolAccount = null;
    if (fromCoinMint === NATIVE_SOL.mintAddress) {
        let lamports;
        if (side === 'buy') {
        ...
        wrappedSolAccount = await createTokenAccountIfNotExist(connection, wrappedSolAccount.toString(), owner, TOKENS.WSOL.mintAddress, lamports, transaction, signers);
    }
    ...
    if (market.baseMintAddress.toBase58() === toMint &&
    ...
    return await sendTransaction(connection, wallet, mergeTransactions([transaction, settleTransactions.transaction]), [...signers, ...settleTransactions.signers]);
}
...
export function routeSwapInInstruction(programId, ammProgramId, fromAmmId, toAmmId, ammAuthority, ammOpenOrders, _ammTargetOrders, poolCoinTokenAccount, poolPcTokenAccount, 
// serum
serumProgramId, serumMarket, serumBids, serumAsks, serumEventQueue, serumCoinVaultAccount, serumPcVaultAccount, serumVaultSigner, 
// user
...
export function routeSwapOutInstruction(programId, ammProgramId, fromAmmId, toAmmId, ammAuthority, ammOpenOrders, _ammTargetOrders, poolCoinTokenAccount, poolPcTokenAccount, 
// serum
serumProgramId, serumMarket, serumBids, serumAsks, serumEventQueue, serumCoinVaultAccount, serumPcVaultAccount, serumVaultSigner, 
// user
...
export async function settleFund(connection, market, openOrders, wallet, baseMint, quoteMint, baseWallet, quoteWallet) {
    ...
    const { transaction, signers } = await market.makeSettleFundsTransaction(connection, openOrders, wrappedBaseAccount ?? new PublicKey(baseWallet), wrappedQuoteAccount ?? new PublicKey(quoteWallet), quoteToken && quoteToken.referrer
