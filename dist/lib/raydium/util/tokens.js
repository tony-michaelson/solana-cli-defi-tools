...
export const TOKENS = {
    ...
    GENE: {
        ...
        details: 'Genopets is the world’s first Free-to-Play, Move-to-Earn NFT game that makes it fun and rewarding to live an active lifestyle. Build on Solana, Genopets integrates your daily activity in real life with blockchain Play-to-Earn economics so you can turn your real-life actions into expansive gameplay and earn crypto while doing it.',
        docs: {
    ...
    DFL: {
        ...
        details: 'DeFi Land is a multi-chain agriculture simulation web game created to gamify decentralized finance. The game will have all the features that traditional platforms have, but it will be gathered all in one place. DeFi Land gamifies decentralized finance by turning investment activities into games.',
        docs: {
    ...
    TTT: {
        ...
        details: 'TabTrader is a trading terminal that supports 34 exchanges with over 12,000 instruments, a variety of analytical tools, and a convenient interface. It’s a quick-access application allowing you to track your exchange accounts, trade, analyze charts, and get instantly notified on price changes, all within one unified interface. The app has a rating of 4.7 on the Apple App Store (with over 52,000 ratings) and a rating of 4.5 on the Google Play Store (with over 55,000 ratings).',
        docs: {
    ...
    RUN: {
        ...
        details: 'RunNode is a bridge from web 2.0 to 3.0 and powers the infrastructure of Solana through its RPC protocol. With a quick onboarding application to get an RPC endpoint in under 30 seconds, any project can launch, build and scale its Solana dApp — now.',
        docs: {
    ...
    REAL: {
        ...
        details: 'Realy Metaverse will be the 1st Live-to-Earn metaverse on Solana. Developed via Unreal Engine, Realy is a virtual city with AAA graphics that seamlessly integrates virtual and reality.',
        docs: {
...
function blockBlackList(tokens) {
...
const notUseSolanaPicMint = [TOKENS.TTT.mintAddress];
function flushTokenIcon(tokens) {
...
function addTokensSolanaFunc(tokens) {
    tokens.forEach((itemToken) => {
        ...
        if (!Object.values(TOKENS).find((item) => item.mintAddress === itemToken.address)) {
            ...
            if (token.symbol !== itemToken.symbol &&
