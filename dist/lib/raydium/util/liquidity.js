...
import { TOKEN_PROGRAM_ID } from './ids.js';
import { TokenAmount } from './safe-math.js';
import { NATIVE_SOL, TOKENS } from './tokens.js';
import { createAssociatedTokenAccountIfNotExist, createTokenAccountIfNotExist, sendTransaction, } from './web3.js';
...
export function getOutAmount(poolInfo, amount, fromCoinMint, toCoinMint, slippage) {
    ...
    else if (fromCoinMint === pc.mintAddress &&
...
export function getOutAmountStable(poolInfo, amount, fromCoinMint, toCoinMint, slippage) {
...
export async function addLiquidity(connection, wallet, poolInfo, fromCoinAccount, toCoinAccount, lpAccount, fromCoin, toCoin, fromAmount, toAmount, fixedCoin) {
    ...
    if (poolInfo.coin.mintAddress === toCoin.mintAddress &&
...
serumMarket, 
// user
userCoinTokenAccount, userPcTokenAccount, userLpTokenAccount, userOwner, marketEventQ, maxCoinAmount, maxPcAmount, fixedFromCoin) {
    const dataLayout = struct([
        u8('instruction'),
        nu64('maxCoinAmount'),
        nu64('maxPcAmount'),
    ...
    const keys = [
        ...
        { pubkey: userLpTokenAccount, isSigner: false, isWritable: true },
        { pubkey: userOwner, isSigner: true, isWritable: false },
        { pubkey: marketEventQ, isSigner: false, isWritable: false },
    ];
    const data = Buffer.alloc(dataLayout.span);
    dataLayout.encode({
        instruction: 3,
...
serumProgramId, serumMarket, serumCoinVaultAccount, serumPcVaultAccount, serumVaultSigner, 
// user
userLpTokenAccount, userCoinTokenAccount, userPcTokenAccount, userOwner, poolInfo, amount) {
    const dataLayout = struct([u8('instruction'), nu64('amount')]);
    const keys = [
        { pubkey: TOKEN_PROGRAM_ID, isSigner: false, isWritable: false },
        { pubkey: ammId, isSigner: false, isWritable: true },
