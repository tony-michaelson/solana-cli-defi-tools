...
describe('calcSwapAmountForDesiredPrice', () => {
    it('[orca] calculates proper swap amount for under peg (deep pool)', () => {
        ...
        const swapAmount = calcSwapAmountForDesiredPrice('orca', baseAmount, 9, quoteAmount, 6, desiredPrice, tradeFee, ownerFee);
        ...
        expect(newPrice.gte(desiredPrice.sub('0.00001')) &&
            newPrice.lte(desiredPrice.add('0.00001'))).toBeTruthy();
    });
    it('[raydium] calculates proper swap amount for under peg (deep pool)', () => {
        ...
        const desiredPrice = new Decimal('48.28');
        const tradeFee = new Fee('25', '10000');
        const swapAmount = calcSwapAmountForDesiredPrice('raydium', baseAmount, 9, quoteAmount, 6, desiredPrice, tradeFee);
        console.log(swapAmount);
        const newPrice = swapAmount.expect.price;
        expect(swapAmount.input.toString()).toStrictEqual('base');
        // new price within + or - 0.00001 of desired price
...
describe('raydiumSwapQuote', () => {
    it('calculates correct swap quote for input token base side', () => {
        ...
        const expectedQuote = new Decimal('9440449.460135').toFixed(decimalPrecision);
        expect(quote.newQuoteAmount.toFixed(decimalPrecision)).toStrictEqual(expectedQuote);
        const expectedBase = new Decimal('189587.202094932').toFixed(decimalPrecision);
        expect(quote.newBaseAmount.toFixed(decimalPrecision)).toStrictEqual(expectedBase);
    });
    it('calculates correct swap quote for input token quote side', () => {
        ...
        console.log(quote);
        const decimalPrecision = 4;
        const expectedQuote = new Decimal('9445449.460135').toFixed(decimalPrecision);
        expect(quote.newQuoteAmount.toFixed(decimalPrecision)).toStrictEqual(expectedQuote);
        const expectedBase = new Decimal('189487.09384431').toFixed(decimalPrecision);
        expect(quote.newBaseAmount.toFixed(decimalPrecision)).toStrictEqual(expectedBase);
    });
});
