...
import { createTokenMint, findAssociatedTokenAddress, mintTo, } from '../lib/token/token.js';
import { TOKEN_PROGRAM_ID, Token, u64 } from '@solana/spl-token';
import { checkOptions, createKeypairFromFile, getConnection, getNetworkName, printAccountBalance, } from '../lib/util.js';
async function getAccountBalance(accountStr, option) {
...
async function getAssociatedTokenAddress(mintPubKey, ownerPubKey, option) {
    checkOptions(option, 'network');
    const networkName = getNetworkName(option);
    const connection = getConnection(networkName);
...
async function createMint(authority, decimals, option) {
    ...
    if (mintDecimals >= 1 && mintDecimals <= 9) {
        const mintPK = await createTokenMint(connection, payer, keyfile, mintAuthority, mintDecimals);
        ...
        if ('createUserATA' in option) {
            ...
            await mintTo(connection, payer, mintPK, tokenAccount, mintAuthority, shiftedAmount);
...
async function createPDA(publicKey, programId, option) {
    ...
    const [authority, nonce] = await PublicKey.findProgramAddress(seeds, programIdPK);
    console.log('Authority:', authority.toString());
    console.log('Bump Seed:', nonce);
}
async function createSPLTokenAccount(mintKey, ownerKey, option) {
...
export async function createATAAccount(mintKey, ownerKey, option) {
    checkOptions(option, 'network', 'payer');
    const networkName = getNetworkName(option);
    const connection = getConnection(networkName);
    const payer = await createKeypairFromFile(option['payer']);
...
export async function transferTokens(mintStr, fromStr, amountStr, toStr, option) {
    checkOptions(option, 'network', 'payer');
    const networkName = getNetworkName(option);
    const connection = getConnection(networkName);
    const payer = await createKeypairFromFile(option['payer']);
    ...
    const signature = await mint.transfer(from, to, payer.publicKey, [payer], new u64(shiftedAmount.toString()));
    console.log('txid:', signature);
}
async function mintToCmd(mintKey, accountKey, amountStr, option) {
    checkOptions(option, 'network', 'payer');
    const networkName = getNetworkName(option);
    const connection = getConnection(networkName);
    const payer = await createKeypairFromFile(option['payer']);
    const signer = 'authoritySigner' in option
    ...
    if (parseInt(amountStr) > 0) {
        ...
        console.log('Amount To Mint (after shifting by mint decimal places):', shiftedAmount.toString());
        if (account) {
            ...
            if ('authority' in option) {
                ...
                await mintTo(connection, payer, mintPK, acctPK, mintAuthorityPK, shiftedAmount);
            ...
            const accountBalanceNew = (await connection.getTokenAccountBalance(acctPK)).value;
...
export function addTokenCommands(program) {
