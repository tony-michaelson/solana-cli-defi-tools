import { networkOption, payerOption, quoteOnlyOption, SABER_SWAP_SLIPPAGE, swapDirectionOption, } from '../lib/constants.js';
...
import { executeInstruction, getExchange, initializeStableSwap, printSaberPoolATABalances, } from '../lib/saber/saber.js';
const { StableSwap, SWAP_PROGRAM_ID, RECOMMENDED_FEES } = saber;
async function createPool(mintAStr, tokenAmountAStr, mintBStr, tokenAmountBStr, option) {
    ...
    const [authority, nonce] = await PublicKey.findProgramAddress([swapAccount.publicKey.toBuffer()], SWAP_PROGRAM_ID);
    ...
    const mintLP = await Token.createMint(connection, payer, authority, null, 2, TOKEN_PROGRAM_ID);
    ...
    initializeStableSwap(connection, payer, swapAccount, transferInstructions, initSwapInstruction);
}
async function showPool(tokenSwapID, option) {
    ...
    const tokenAccountBalA = (await connection.getTokenAccountBalance(tokenSwap.state.tokenA.reserve)).value.amount;
    const tokenAccountBalB = (await connection.getTokenAccountBalance(tokenSwap.state.tokenB.reserve)).value.amount;
...
async function deposit(tokenSwapID, tokenQtyA, tokenQtyB, option) {
    checkOptions(option, 'network', 'payer');
    const networkName = getNetworkName(option);
    const connection = getConnection(networkName);
    const payer = await createKeypairFromFile(option['payer']);
    ...
    const mintLP = new Token(connection, tokenSwap.state.poolTokenMint, TOKEN_PROGRAM_ID, payer);
    const exchangeInfo = await getExchange(connection, networkName, mintA, mintB, mintLP, tokenSwap);
    let minimumPoolTokenAmount;
    if (exchangeInfo.lpTotalSupply.toU64().toNumber() > 0) {
        const minAmountOut = saber.calculateEstimatedMintAmount(exchangeInfo, JSBI.BigInt(tokenAmountA), JSBI.BigInt(tokenAmountB));
    ...
    console.log('--- / DEPOSIT QUOTE ---');
    const printBal = async () => await printSaberPoolATABalances(connection, tokenSwap, payer);
    if (!('quoteOnly' in option)) {
        executeInstruction(connection, instruction, printBal, printBal, [payer, userAuthority], payer);
    }
}
...
async function withdraw(tokenSwapID, tokenQty, option) {
    ...
    const ataB = await findAssociatedTokenAddress(payer.publicKey, mintB.publicKey);
    const ataLP = await findAssociatedTokenAddress(payer.publicKey, mintLP.publicKey);
    const withdrawTokenAmount = 'all' in option
        ? parseInt((await connection.getTokenAccountBalance(ataLP)).value.amount)
        : parseInt(tokenQty);
    const userAuthority = new Keypair();
    await mintLP.approve(ataLP, userAuthority.publicKey, payer, [], withdrawTokenAmount);
    const withdrawQuote = await saber.calculateEstimatedWithdrawAmount({
        ...exchangeInfo,
        poolTokenAmount: new TokenAmount(exchangeInfo.lpTotalSupply.token, JSBI.BigInt(withdrawTokenAmount)),
    });
...
async function swap(tokenSwapID, tokenQty, option) {
    ...
    const ataA = await findAssociatedTokenAddress(payer.publicKey, mintA.publicKey);
    const ataB = await findAssociatedTokenAddress(payer.publicKey, mintB.publicKey);
    const fromTokenName = option['direction'] === 'AtB' ? 'A' : 'B';
    const toTokenName = option['direction'] === 'AtB' ? 'B' : 'A';
    const fromToken = option['direction'] === 'AtB'
        ? exchangeInfo.reserves[0].amount.token
        : exchangeInfo.reserves[1].amount.token;
    const poolSource = option['direction'] === 'AtB'
        ? tokenSwap.state.tokenA.reserve
        : tokenSwap.state.tokenB.reserve;
    const poolDestination = option['direction'] === 'AtB'
        ? tokenSwap.state.tokenB.reserve
        : tokenSwap.state.tokenA.reserve;
    const adminDestination = option['direction'] === 'AtB'
        ? tokenSwap.state.tokenB.adminFeeAccount
        : tokenSwap.state.tokenA.adminFeeAccount;
    const sourceMint = option['direction'] === 'AtB' ? mintA : mintB;
    const userSource = option['direction'] === 'AtB' ? ataA : ataB;
    const userDestination = option['direction'] === 'AtB' ? ataB : ataA;
    const userAuthority = new Keypair();
    await sourceMint.approve(userSource, userAuthority.publicKey, payer, [], tokenAmount);
    const swapQuote = saber.calculateEstimatedSwapOutputAmount(exchangeInfo, new TokenAmount(fromToken, JSBI.BigInt(tokenAmount)));
...
async function price(tokenSwapID, option) {
    checkOptions(option, 'network', 'payer');
    const networkName = getNetworkName(option);
    const connection = getConnection(networkName);
    const payer = await createKeypairFromFile(option['payer']);
...
export function addSaberCommands(program) {
    const saber = program.command('saber');
    saber
        .command('createPool <mintA> <tokenAInitialAmount> <mintB> <tokenBInitialAmount>')
